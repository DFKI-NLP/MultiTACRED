...
@description: reorganized backtranslated .jsonl file
"""
...
def change_keys(logger, input_file, output_file, overwrite):
    ...
    with open(input_file, "r", encoding="utf-8") as f_in, open(
        ...
        for line in f_in:
            ...
            example["tokens_backtranslated"] = example["tokens_translated"]
            example["entities_backtranslated"] = example["entities_translated"]
            example["language_backtranslated"] = example["language_translated"]

            example["tokens_translated"] = example["tokens"]
            example["entities_translated"] = example["entities"]
            example["language_translated"] = example["language"]

            example["tokens"] = example.pop("tokens_original")
            example["entities"] = example.pop("entities_original")
            example["language"] = example.pop("language_original")
...
def main():
    ...
    # parse args
    parser = argparse.ArgumentParser(description="Script to post process backtranslated dataset")
    # Needed
